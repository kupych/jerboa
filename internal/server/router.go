...
func NewRouter(cfg *config.Config, queries *db.Queries, authProvider *auth.Provider, store *storage.Store, s3, mirrorS3 *storage.S3Client, processor *audio.Processor, webFS fs.FS) http.Handler {
	...
	r.Group(func(r chi.Router) {
		...
		r.Patch("/api/bands/{slug}/tracks/{trackID}/mute", trackH.UpdateMuted)
		...
		r.Post("/api/bands/{slug}/tracks/{trackID}/overdubs/link", overdubH.LinkOverdub)
		...
		r.Get("/api/bands/{slug}/files/multipart/parts", fileH.ListParts)
